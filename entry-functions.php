...
date_default_timezone_set('Asia/Tehran') ;
	...
	function mf_get_entry_values($dbh,$form_id,$entry_id,$use_review_table=false,$options=array()){
		...
		foreach ($form_elements as $element){
			...
			if('simple_name' == $element_type){ //Simple Name - 2 elements
                ...
                $phone_1 = substr($phone_value,1,2);
				$phone_2 = substr($phone_value,4,2);
		...
		foreach ($form_elements as $element){
			...
			if('simple_name' == $element_type){ //Simple Name - 2 elements
				...
				if(!empty($phone_value)){
					$entry_details[$i]['value'] = "{$phone_3}/{$phone_2}/{$phone_1}";
				}
	...
	function mf_display_entries_table($dbh,$form_id,$options){
		...
		$column_name_lookup['date_created']   = 'تاریخ ایجاد';
		$column_name_lookup['date_updated']   = 'تاریخ بروز رسانی';
		$column_name_lookup['ip_address'] 	  = 'آی پی آدرس';
		/* 
		$column_type_lookup['id'] 			= 'عدد';
		$column_type_lookup['row_num']		= 'عدد';
		$column_type_lookup['date_created'] = 'تاریخ';
		$column_type_lookup['date_updated'] = 'تاریخ';
		$column_type_lookup['ip_address'] 	= 'متن';
		 */
		...
		while($row = mf_do_fetch_result($sth)){
			...
			if('address' == $element_type){ //address has 6 fields
				$column_name_lookup['element_'.$row['element_id'].'_1'] = $row['element_title'].' - آدرس خیابان';
				$column_name_lookup['element_'.$row['element_id'].'_2'] = 'آدرس خط 2';
				$column_name_lookup['element_'.$row['element_id'].'_3'] = 'شهر';
				$column_name_lookup['element_'.$row['element_id'].'_4'] = 'استان / ولایت / منطقه';
				$column_name_lookup['element_'.$row['element_id'].'_5'] = 'پستی /کد پستی';
				$column_name_lookup['element_'.$row['element_id'].'_6'] = 'کشور';
				...
				$column_name_lookup['element_'.$row['element_id'].'_1'] = $row['element_title'].' - ابتدا';
				$column_name_lookup['element_'.$row['element_id'].'_2'] = $row['element_title'].' - انتها';
				...
				$column_name_lookup['element_'.$row['element_id'].'_1'] = $row['element_title'].' - ابتدا';
				$column_name_lookup['element_'.$row['element_id'].'_2'] = $row['element_title'].' - میانه';
				$column_name_lookup['element_'.$row['element_id'].'_3'] = $row['element_title'].' - انتها';
				...
				$column_name_lookup['element_'.$row['element_id'].'_1'] = $row['element_title'].' - عنوان';
				$column_name_lookup['element_'.$row['element_id'].'_2'] = $row['element_title'].' - ابتدا';
				$column_name_lookup['element_'.$row['element_id'].'_3'] = $row['element_title'].' - انتها';
				$column_name_lookup['element_'.$row['element_id'].'_4'] = $row['element_title'].' - پسوند';
				...
				$column_name_lookup['element_'.$row['element_id'].'_1'] = $row['element_title'].' - عنوان';
				$column_name_lookup['element_'.$row['element_id'].'_2'] = $row['element_title'].' - ابتدا';
				$column_name_lookup['element_'.$row['element_id'].'_3'] = $row['element_title'].' - میانه';
				$column_name_lookup['element_'.$row['element_id'].'_4'] = $row['element_title'].' - انتها';
				$column_name_lookup['element_'.$row['element_id'].'_5'] = $row['element_title'].' - پسوند';
			...
			foreach($column_prefs as $column_name){
				...
				if($column_type_lookup[$column_name] == 'time'){
					...
					if(!empty($row[$column_name]) && ($row[$column_name] != '0000-00-00')){
						...
						$form_data[$i][$j]  = ($row[$column_name] != '') ? jdate('Y/n/j', strtotime($row[$column_name]), '', 'Asia/Tehran') : '';
					}

					if($column_name == 'date_created' || $column_name == 'date_updated'){
						//$form_data[$i][$j] = mf_short_relative_date($row[$column_name]);
						$form_data[$i][$j] = ($row[$column_name] != '') ? jdate('H:i:s ,Y/n/j', strtotime($row[$column_name]), '', 'Asia/Tehran') : '';
					}
					...
					if(!empty($row[$column_name])){
						if($column_type_lookup[$column_name] == 'phone'){
							$form_data[$i][$j] = ''.substr($row[$column_name],6,4).'/'.substr($row[$column_name],4,2).'/'.substr($row[$column_name],1,2);
						}else{
							$form_data[$i][$j] = $row[$column_name];
						}
					}
		...
		if(!empty($form_data)){
			...
			$table_body_markup .= "<tr><td colspan=\"".count($column_labels)."\"> <div id=\"filter_no_results\"><h3>جستجوی شما نتیجه ای نداشت.</h3></div></td></tr>";
		}
		...
		if(!empty($lastpage) && $numrows > $rows_per_page){
			...
			if ($pageno != 1) {
			   if($lastpage > 13 && $pageno > 7){	
			   		$pagination_markup .= "<li class=\"page\"><a href='{$self_path}?id={$form_id}&pageno=1'>&#8676; ابتدا</a></li>";
			   }
			   $prevpage = $pageno-1;
			} 
			...
			if ($pageno != $lastpage) 
			{
			   $nextpage = $pageno+1;
			   if($lastpage > 13){
			   		$pagination_markup .= "<li class=\"page\"><a href='{$self_path}?id={$form_id}&pageno=$lastpage'>قبلی &#8677;</a></li>";
			   }
			}
			
			$pagination_markup = '<ul class="pages bluesoft small" id="me_pagination">'.$pagination_markup.'</ul>';
			$pagination_markup .= "<div id=\"me_pagination_label\">در حال نمایش <strong>{$first_row_number}-{$last_row_number}</strong> از <strong id=\"me_entries_total\">{$numrows}</strong> ورودی ها</div>";
		}else{
...
/* 
	...
	function mf_get_filtered_entries_ids($dbh,$form_id,$options=array()){
		...
		$column_name_lookup['date_updated'] = 'تاریخ بروز رسانی';
		$column_name_lookup['ip_address'] 	= 'آی پی آدرس';
		$column_name_lookup['payment_amount'] = 'مبلغ پرداختی';
		$column_name_lookup['payment_status'] = 'وضعیت پرداخت';
		$column_name_lookup['payment_id']	  = 'آی دی پرداخت';
		/* 
		$column_type_lookup['id'] 			= 'عدد';
		$column_type_lookup['row_num']		= 'عدد';
		$column_type_lookup['date_created'] = 'تاریخ';
		$column_type_lookup['date_updated'] = 'تاریخ';
		$column_type_lookup['ip_address'] 	= 'متن';
		$column_type_lookup['payment_amount'] = 'پول';
		$column_type_lookup['payment_status'] = 'payment_status';
		$column_type_lookup['payment_id']	  = 'متن';
